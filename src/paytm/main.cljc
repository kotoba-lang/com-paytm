(ns paytm.main
  "Kotodama WASM entrypoint for the Paytm clean-room actor (L4) — Clojure port.
  ...
  facts (`paytm.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"paytm.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "paytm-compat" :tier tier :entities entities} 200])
